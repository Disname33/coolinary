...
NEWEST_GENERATION_FOR_GUEST = 13  # guest users only see generations older than this
root_url = "/familytree/"
...
show_by_branch = bool(branch1_name)
...
def index(request):  # dashboard page
    ...
    browser = None
    show_mobile = request.GET.get("show_mobile")
...
def get_display_profile(request):
    ...
    if not len(profile):
        ...
        person.save()
        profile = Profile.objects.create(user=request.user, person=person)
        profile.save()
        profile = Profile.objects.filter(user=request.user)
