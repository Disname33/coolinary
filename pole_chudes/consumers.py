...
class GameConsumer(ObserverModelInstanceMixin, GenericAsyncAPIConsumer):
    ...
    def user_is_active_player(self, room: Round):
        return room.get_active_player().user == self.scope['user']

    @database_sync_to_async
    def get_word(self, room: Round):
        return room.riddle.word

    @database_sync_to_async
    def is_creator_running_bot(self, room: Round):
    ...
    def get_active_player_name(self, pk: int) -> str:
        return Round.objects.get(pk=pk).get_active_player().name

    @database_sync_to_async
    def add_user_to_room(self, pk):
        user: User = self.scope["user"]
        room = Round.objects.get(pk=pk)
        if not room.is_complete and not room.is_user_in_game(user):
